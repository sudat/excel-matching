...
async def test_database_connection():
    ...
    if supabase:
        try:
            ...
            response = (
                supabase.table("business_requests")
                .select("*", count=cast(CountMethodType, "exact"))
                .execute()
            )
            files_response = (
    ...
    if not engine:
        raise HTTPException(
            status_code=500,
            detail="Neither Supabase client nor database engine configured",
        )
    ...
    try:
        ...
        error_msg = (
            f"All database connection methods failed. SQLAlchemy error: {str(e)}"
        )
...
async def test_pinecone_connection():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Pinecone connection failed: {str(e)}"
        )
...
async def simple_database_test():
    ...
    if result["status"] == "success":
        ...
        raise HTTPException(
            status_code=500, detail=result.get("error", "Database connection failed")
        )
...
async def get_business_requests():
    ...
    if result["status"] == "success":
        return {"status": "success", "data": result["data"], "method": result["method"]}
    else:
        raise HTTPException(
            status_code=500,
            detail=result.get("error", "Failed to fetch business requests"),
        )
...
async def create_business_request(title: str, description: str = ""):
    ...
    if result["status"] == "success":
        ...
        raise HTTPException(
            status_code=500,
            detail=result.get("error", "Failed to create business request"),
        )
